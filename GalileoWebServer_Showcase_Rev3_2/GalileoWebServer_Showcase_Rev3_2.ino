...
#define PAGE_HOME 0
#define PAGE_TELEMETRY 1
#define PAGE_COMMAND 2
...
String version = "Rev3.2"; // Update version info to make sure the correct sketch is loaded
...
void loop() {
  ...
  // TelemetryPrint( currentTime );

  ledAttract(currentTime); // This function causes the LEDs to change color every 5 seconds

  // Run grow light LEDs
  // If the temperature goes over 90 turn them off
  growLED(currentTime);

  //If the humidity is below 300 run the mist maker
  checkHumidityAutomation(currentTime);
  ...
  if (client) {
    ...
    while (client.connected()) {
      if (client.available()) {
        ...
        if (c == '\n' && currentLineIsBlank) {
          if(controlString.indexOf("?TELEMETRY") > -1)
            TelemetryWebPrint(currentTime, client);
          else
...
/*
***************************************************************************
A Function for Reporting Telemetry
***************************************************************************
*/

void TelemetryPrint(unsigned long currentTime) 
{
  if (currentTime - lastTelemetryTime > 5000)
  {
    ...
    Serial.print("  Motion: ");
  int pirReading = analogRead(3);
  if ( pirReading > 300 ) { Serial.print("activated"); }
  else { Serial.print("no motion"); }
    Serial.print("  Door: ");
    if (position_door < 45) { Serial.print("closed"); }
    else { Serial.print("open"); }
    Serial.print("  Left Window: ");
    if (position_left_window < 15) { Serial.print("closed"); }
    else { Serial.print("open"); }
    Serial.print("  Right Window: ");
    if (position_right_window < 15) { Serial.print("closed"); }
    else { Serial.print("open"); }
...
/*
***************************************************************************
A Function for Reporting Telemetry via Web Request for IoT Hub
***************************************************************************
*/

void TelemetryWebPrint(unsigned long currentTime, EthernetClient client) 
{
  // send a standard http response header
  client.println("HTTP/1.1 200 OK");
  ...
  client.print("Motion: ");
  int pirReading = analogRead(3);
  if ( pirReading > 300 ) { client.print("activated"); }
  else { client.print("no motion"); }
  client.print(", ");
  client.print("Door: ");
  if (position_door < 45) { client.print("closed"); }
  else { client.print("open"); }
  client.print(", ");
  client.print("Left Window: ");
  if (position_left_window < 15) { client.print("closed"); }
  else { client.print("open"); }
  client.print(", ");
  client.print("Right Window: ");
  if (position_right_window < 15) { client.print("closed"); }
  else { client.print("open"); }
  client.print(", ");
  client.print("Grow Light: ");
  client.print(relayState[1]);
  client.print(", ");
  client.print("Fan: ");
  client.print(relayState[3]);
  client.print(", ");
  client.print("Mist Maker: ");
...
/*
***************************************************************************
A Function to Control the Mist Maker based on Humidity
***************************************************************************
*/

void checkHumidityAutomation(unsigned long currentTime) {
  int humReading = analogRead(1);
  if ((humReading < 300)  && ((relayState[0]) == LOW)) {  // If humidity is low & the relay is off
    if ( currentTime - previousTimeRelay1 > 5000 ) {  // If enough time has elapsed
      if ( analogRead(0) < 100) { // If the water level is high enough
        digitalWrite(relayPin[0],HIGH);
        relayState[0] = HIGH;
        previousTimeRelay1 = currentTime;
        Serial.println("Mist Maker is on.");
      }
    }
  }
  else;
  if ((humReading > 350)  && ((relayState[0]) == HIGH)) {
    if ( currentTime - previousTimeRelay1 > 5000 ) {
      digitalWrite(relayPin[0],LOW);
      relayState[0] = LOW;
      previousTimeRelay1 = currentTime;
      Serial.println("Mist Maker is off.");
    }
  }
}
...
/*
***************************************************************************
A Function to Control the Grow Light Based on Temperature
***************************************************************************
*/

void growLED(unsigned long currentTime) {
  int tempReading = analogRead(2);
  float tempValue = -0.00277358191992865 * pow(tempReading ,2) + 2.12540388128839 * tempReading + -183.108886107634;
  if ((tempValue < 85.00)  && ((relayState[1]) == LOW)) {
    if ( currentTime - previousTimeRelay2 > 2000 ) {
      digitalWrite(relayPin[1],HIGH);
      relayState[1] = HIGH;
      previousTimeRelay2 = currentTime;
      Serial.println("LED Grow Light is on.");
    }
  }
  else;
  if ((tempValue > 95.00)  && ((relayState[1]) == HIGH)) {
    if ( currentTime - previousTimeRelay2 > 2000 ) {
      digitalWrite(relayPin[1],LOW);
      relayState[1] = LOW;
      previousTimeRelay2 = currentTime;
      Serial.println("LED Grow Light is off.");
    }
  }
}
...
void tempCheckAutomation(unsigned long currentTime) {
  int tempReading = analogRead(2);
  float tempValue = -0.00277358191992865 * pow(tempReading ,2) + 2.12540388128839 * tempReading + -183.108886107634;
  if ((tempValue > 95.00)  && ((relayState[3]) == LOW)) {
    if ( currentTime - previousTimeRelay4 > 2000 ) {
      digitalWrite(relayPin[3],HIGH);
      relayState[3] = HIGH;
      previousTimeRelay4 = currentTime;
  ...
  if ((tempValue < 90.00)  && ((relayState[3]) == HIGH)) {
    if ( currentTime - previousTimeRelay4 > 2000 ) {
      digitalWrite(relayPin[3],LOW);
      relayState[3] = LOW;
      previousTimeRelay4 = currentTime;
...
void open_left_window(unsigned long currentTime) {
  if ( currentTime - previousTimeLeftWindow > 2000 ) {
    ...
    Serial.println("Left Window is open.");
  }
}
...
void close_left_window(unsigned long currentTime) {
  if ( currentTime - previousTimeLeftWindow > 2000 ) {
    ...
    servo_left_window.write(85);
    previousTimeLeftWindow = currentTime;
    Serial.println("Left Window is closed.");
  }
}
...
void open_right_window(unsigned long currentTime) {
  if ( currentTime - previousTimeRightWindow > 2000 ) {
    ...
    Serial.println("Right Window is open.");
  }
}
...
void close_right_window(unsigned long currentTime) {
  if ( currentTime - previousTimeRightWindow > 2000 ) {
    ...
    servo_right_window.write(110);
    previousTimeRightWindow = currentTime;
    Serial.println("Right Window is closed.");
  }
}
